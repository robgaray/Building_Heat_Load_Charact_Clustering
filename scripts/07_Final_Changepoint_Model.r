...
Dat$Power_original<-Dat$Power
Dat$Power<-Dat$Power_corrected
...
Dat$Power_fitted_TOW<-Dat$Power_fitted

rm(Dat.output, opt, Changepoint_Parameters_i, Dat.subs.hw, i)

# rename output
Changepoint_Pars_summ_TOW2<-Changepoint_Pars_summ_TOW
rm(Changepoint_Pars_summ_TOW)

# Write to file
setwd(paste(WD, "output", sep="/"))
write.csv(Changepoint_Pars_summ_TOW2,"07_Changepoint_Pars_summ_TOW2.csv")
setwd(WD)